...
int main(int argc, char* argv[])
{
  ...
  SUBSPACE_FLUID_3D_COMPRESSED_EIGEN subspaceFluid(xRes, yRes, zRes, reducedPath, &boundaries[0], usingIOP, true);
  subspaceFluid.loadCubatureTrainingBases();

  CUBATURE_GENERATOR_COMPRESSED_EIGEN generator(&subspaceFluid);
  ...
  auto U_preadvect_data = subspaceFluid.U_preadvect_data();
  // DEBUG
  ...
  if (file == NULL)
  {
    ...
    {
      printf("Inside reduced snapshots loop: x = %i\n", x);
      VectorXd preadvect, prediffuse;
      VectorXd q;

      EIGEN::readRaw(filePreadvect, cols, preadvect);
      EIGEN::readRaw(filePrediffuse, cols, prediffuse);
      auto preadvectField = VECTOR3_FIELD_3D(xRes, yRes, zRes);
      preadvectField.setWithPeeled(preadvect);
